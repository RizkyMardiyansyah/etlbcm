# =============================================================================
# sync_atrium_software.py
# Sinkronisasi tabel Atrium_CMDB_Software: bcmdb (sa) -> GRAFANADB (grafanauser)
# Logic : SELECT dari bcmdb -> MERGE (UPSERT) ke GRAFANADB
#         Composite key: DeviceID + InventoryID
...
LOG_FILE = f"sync_atrium_software_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
TABLE_KEY  = "Atrium_CMDB_Software"
SCHEMA     = TABLE_SCHEMAS[TABLE_KEY]
COLUMNS    = SCHEMA["columns"]
COL_NAMES  = [c[0] for c in COLUMNS]
MERGE_KEYS = SCHEMA["merge_keys"]   # ["DeviceID", "InventoryID"]
...
"""
    ...
    for row in rows:
        ...
        # DeviceID dan InventoryID tidak boleh NULL (composite PK)
        if values[key_indices["DeviceID"]] is None or values[key_indices["InventoryID"]] is None:
            skipped += 1
            continue
        ...
        try:
            ...
            errors += 1
            log.error(f"[upsert] Gagal pada DeviceID={row[0]}, InventoryID={row[1]}: {e}")
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_atrium_software (Atrium_CMDB_Software) -- {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_atrium_software (Atrium_CMDB_Software) -- {datetime.now()}")
    log.info("=" * 60)
