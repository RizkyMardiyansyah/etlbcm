...
# Tabel: Atrium_CMDB_Software | composite key: DeviceID + InventoryID
# =============================================================================
...
LOG_FILE = os.path.join(LOG_DIR, f"sync_atrium_software_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_atrium_software — {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_atrium_software — {datetime.now()}")
    log.info("=" * 60)
