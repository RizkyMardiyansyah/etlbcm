# =============================================================================
# sync_famview.py
# Sinkronisasi tabel V_CMDB_FAM: bcmdb (sa) -> GRAFANADB (grafanauser)
...
# ─── Logging ─────────────────────────────────────────────────────────────────
LOG_FILE = f"sync_famview_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def ensure_table_exists(cursor: pyodbc.Cursor, table_name: str) -> None:
    """Buat tabel FAMView jika belum ada."""
    col_defs = ",\n    ".join(
...
def fetch_source_data(cursor: pyodbc.Cursor, table_name: str) -> list:
    """
    FAMView kemungkinan adalah VIEW di bcmdb, bukan tabel biasa.
    Query tetap menggunakan SELECT biasa -- tidak masalah untuk view maupun tabel.
    """
...
def upsert_rows(cursor: pyodbc.Cursor, table_name: str, rows: list) -> None:
    """
    UPSERT menggunakan MERGE dengan PK = DeviceID.
    - FAMView bisa punya banyak software per DeviceID (1 device : banyak software).
    - Jika relasi tersebut perlu di-flatten ke 1 baris per DeviceID, gunakan logika ini.
    - Jika ingin menyimpan semua record (multi-row per DeviceID), ubah PK strategy.
    """
    if not rows:
...
"""
    ...
    for row in rows:
        ...
        if values[pk_idx] is None:
            skipped += 1
            log.warning(f"[upsert] Baris di-skip karena DeviceID = NULL: {row}")
            continue
        ...
        try:
            ...
            errors += 1
            log.error(f"[upsert] Gagal pada DeviceID={row[pk_idx]}: {e}")
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_famview (V_CMDB_FAM) -- {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_famview (V_CMDB_FAM) -- {datetime.now()}")
    log.info("=" * 60)
