...
LOG_FILE = os.path.join(LOG_DIR, f"sync_famview_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_famview — {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_famview — {datetime.now()}")
    log.info("=" * 60)
