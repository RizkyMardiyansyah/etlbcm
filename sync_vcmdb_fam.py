...
# CATATAN: Tabel ini BERBEDA dengan FAMView (sync_famview.py)
...
LOG_FILE = f"sync_vcmdb_fam_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_vcmdb_fam (V_CMDB_FAM) -- {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_vcmdb_fam (V_CMDB_FAM) -- {datetime.now()}")
    log.info("=" * 60)
