...
# Tabel: V_CMDB_SoftwareRel | composite key: DeviceID + Name + Version
# =============================================================================
...
LOG_FILE = os.path.join(LOG_DIR, f"sync_software_rel_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_software_rel — {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_software_rel — {datetime.now()}")
    log.info("=" * 60)
