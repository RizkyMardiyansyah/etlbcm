# =============================================================================
# sync_computer_system.py
# Sinkronisasi tabel CMDB_ComputerSystem: bcmdb (sa) -> GRAFANADB (grafanauser)
...
# ─── Logging ─────────────────────────────────────────────────────────────────
LOG_FILE = f"sync_computer_system_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
TABLE_KEY = "Atrium_CMDB_ComputerSystem"
SCHEMA    = TABLE_SCHEMAS[TABLE_KEY]
COLUMNS   = SCHEMA["columns"]        # list of (col_name, sql_type)
COL_NAMES = [c[0] for c in COLUMNS]
PK        = SCHEMA["primary_key"]    # "DeviceID"


# ─── Helpers ──────────────────────────────────────────────────────────────────

def get_connection(database: str, user: str, password: str) -> pyodbc.Connection:
    """Buat koneksi pyodbc ke SQL Server."""
    conn_str = (
...
def ensure_table_exists(cursor: pyodbc.Cursor, table_name: str) -> None:
    """Buat tabel di GRAFANADB jika belum ada."""
    col_defs = ",\n    ".join(
...
def fetch_source_data(cursor: pyodbc.Cursor, table_name: str) -> list:
    """Ambil semua baris dari tabel sumber di bcmdb."""
...
def upsert_rows(cursor: pyodbc.Cursor, table_name: str, rows: list) -> None:
    """
    UPSERT menggunakan MERGE statement.
    - Jika baris dengan DeviceID sama sudah ada -> UPDATE semua kolom.
...
"""
    ...
    for row in rows:
        # Konversi None dan string "NULL" menjadi None (SQL NULL)
        values = [None if (v is None or str(v).strip().upper() == "NULL") else v for v in row]
        try:
            cursor.execute(merge_sql, values)
            inserted += 1   # pyodbc tidak membedakan insert/update via rowcount MERGE
        except pyodbc.Error as e:
            errors += 1
            log.error(f"[upsert] Gagal pada DeviceID={row[0]}: {e}")

    cursor.connection.commit()
    log.info(f"[upsert] Selesai. Diproses={inserted}, Error={errors}.")
...
def main():
    ...
    log.info(f"FINISH sync_computer_system — {datetime.now()}")
    log.info("=" * 60)
