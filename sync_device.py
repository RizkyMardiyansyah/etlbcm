# =============================================================================
# sync_device.py
# Sinkronisasi tabel V_CMDB_Device: bcmdb (sa) -> GRAFANADB (grafanauser)
...
LOG_FILE = f"sync_device_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_device (V_CMDB_Device) -- {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_device (V_CMDB_Device) -- {datetime.now()}")
    log.info("=" * 60)
