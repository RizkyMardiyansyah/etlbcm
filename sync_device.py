...
# Tabel: V_CMDB_Device | PK: DeviceID
# =============================================================================
...
LOG_FILE = os.path.join(LOG_DIR, f"sync_device_{datetime.now().strftime('%Y%m%d')}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_device — {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_device — {datetime.now()}")
    log.info("=" * 60)
