# =============================================================================
# sync_software.py
# Sinkronisasi tabel V_CMDB_Software: bcmdb (sa) -> GRAFANADB (grafanauser)
# Logic : SELECT dari bcmdb -> MERGE (UPSERT) ke GRAFANADB
#         Composite key: SoftwareName + Version
...
LOG_FILE = f"sync_software_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
TABLE_KEY  = "V_CMDB_Software"
SCHEMA     = TABLE_SCHEMAS[TABLE_KEY]
COLUMNS    = SCHEMA["columns"]
COL_NAMES  = [c[0] for c in COLUMNS]
MERGE_KEYS = SCHEMA["merge_keys"]   # ["Name", "Version"]
...
"""
    ...
    for row in rows:
        ...
        # Name tidak boleh NULL (bagian PK)
        if values[key_indices["Name"]] is None:
            skipped += 1
            continue

        # Version NULL diganti string kosong agar bisa jadi PK
        ...
        try:
            ...
            errors += 1
            log.error(f"[upsert] Gagal pada SoftwareName={row[0]}: {e}")
...
def main():
    log.info("=" * 60)
    log.info(f"START sync_software (V_CMDB_Software) -- {datetime.now()}")
    log.info("=" * 60)
    ...
    log.info(f"FINISH sync_software (V_CMDB_Software) -- {datetime.now()}")
    log.info("=" * 60)
